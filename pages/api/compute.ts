...
const generateHinglishReport = (formData: any) => {
  ...
  if (careerGoal === "promotion") {
    careerHeadline = "Promotion Yog: Sahi Samay par Sahi Kadam aapko aage le jayega!";
  } else if (careerGoal === "job_change") {
    careerHeadline = "Naukri Badlav Ke Sanket: Naye Raaste, Nayi Oonchaiyan!";
  }
  ...
  return {
    ...
    summary: {
      tone: tone,
      core_focus: `Aane wale 3 mahine aapke liye '${topConcern}' par focus karne ke liye sabse anukool hain. Grahon ki urja is taraf ishara kar rahi hai.`,
    },
    sections: {
      career: {
        ...
        forensic: "Aapki kundali mein Budh (Mercury) ka prabhav communication ko strong banata hai. Iska istemal meetings aur interviews mein karein. Mangal (Mars) ki urja aapko action lene ke liye prerit karegi, lekin jaldbaazi se bachein.",
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  if (req.method === 'POST') {
    try {
      ...
      const report = generateHinglishReport(req.body);
