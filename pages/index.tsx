...
const LoadingSpinner = () => (
  <div className="flex flex-col items-center justify-center min-h-[60vh] text-center">
    <motion.div animate={{ rotate: 360 }} transition={{ duration: 1.5, repeat: Infinity, ease: 'linear' }} className="w-16 h-16 border-4 border-t-brand-gold border-brand-violet rounded-full mb-4" />
...
export default function Home() {
  ...
  const handleFormSubmit = async (formData: any) => {
    ...
    try {
      const response = await fetch('/api/compute', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(formData) });
  ...
  const resetState = () => { setReportData(null); setError(null); setIsLoading(false); }
      ...
      <div className="flex flex-col min-h-screen bg-brand-dark font-sans">
        ...
        <main className="flex-grow container mx-auto px-4 py-8 md:py-12">
          <AnimatePresence mode="wait">
            {isLoading ? (<motion.div key="loader" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}><LoadingSpinner /></motion.div>)
            : error ? (<motion.div key="error" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}><ErrorMessage message={error} onReset={resetState} /></motion.div>)
            : reportData ? (<motion.div key="report" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}><KundaliReport data={reportData} /></motion.div>)
