...
const LoadingSpinner = () => (
  <div className="flex flex-col items-center justify-center min-h-[60vh] text-center">
    ...
    <p className="text-brand-gray mt-2">
      Pandit ji aapke grahon ki sthiti ka vishleshan kar rahe hain.
    </p>
  </div>
);
...
const ErrorMessage = ({ message, onReset }: { message: string; onReset: () => void }) => (
  <div className="flex flex-col items-center justify-center min-h-[60vh] text-center p-4">
    ...
    <button
      onClick={onReset}
      className="px-8 py-3 bg-brand-gold text-brand-dark font-bold rounded-full hover:bg-yellow-400 transition-all"
    >
          ...
          <AnimatePresence mode="wait">
            {isLoading ? (
              ...
              <motion.div key="error" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                <ErrorMessage message={error} onReset={resetState} />
              </motion.div>
            ) : reportData ? (
              <motion.div key="report" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                <KundaliReport data={reportData} />
                {/* Optional raw JSON for quick debug — production me hata dena */}
                <pre className="mt-6 text-xs whitespace-pre-wrap break-all border border-white/10 p-3 rounded-lg bg-black/30">
